...
A complete second design is the site root. v1 (everything above) is unchanged so the two can be
compared. Full spec, tokens, motion inventory and regeneration instructions: **`V2-README.md`**.
Short version: Inter + Caveat, `#0171FD` primary on `#F5F7F9` bands with `#030505` heroes,
Lenis + clip-path mask reveals + staggered `.rv` fades, and every page except `index.html` is
generated by `_build.py` and `_build2.py`.

### v2 craft layer (22 Sep 2026)
The client felt the v2 build looked AI-made, which it did: their reference mockup was AI-generated and building
it faithfully inherited its tells. `assets/css/v2.css` now ends with a **craft layer** that keeps the
approved layout, palette and typefaces and rewrites how each section is composed — hairline rules instead of
wall-to-wall cards, real type-scale contrast, uncropped client photography, per-section motion. Reference
teardowns came from the Framer templates the client's agency shortlisted (Spector, Lateral, Himon, Outline
Studio, Kairn, ClearPath, Trev, Mira). Full before/after table, motion inventory and the gotchas it introduced:
`V2-README.md`.

### Repository layout (22 Sep 2026, after promotion)
- **root** — the live design (13 pages), `assets/css/v2.css`, `assets/js/v2.js`, built by
  `_build.py` + `_build2.py`. Full spec: `V2-README.md`.
- **`v1/`** — the original Devix-derived design (11 pages), kept for comparison. Uses
  `assets/css/style.css` + `assets/js/main.js` and references `../assets/`.
- **`v2/`** — redirect stubs only. Every page that lived at `/v2/<page>.html` while the new design
  was in a subfolder now redirects to `/<page>.html`, so links already handed out keep working.
- `assets/img/` is shared by both designs.
