...
A complete second design lives in `v2/`. v1 (everything above) is unchanged so the two can be
...
generated by `v2/_build.py` and `v2/_build2.py`.
...
it faithfully inherited its tells. `v2/assets/css/v2.css` now ends with a **craft layer** that keeps the
